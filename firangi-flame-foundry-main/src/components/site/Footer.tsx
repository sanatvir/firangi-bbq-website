  ...
  return (
    <footer className="border-t border-border bg-secondary/30 px-4 sm:px-6 py-12 sm:py-16 lg:px-12">
      <div className="mx-auto grid max-w-7xl gap-8 sm:gap-10 sm:grid-cols-2 lg:grid-cols-3">
        <div>
          <div className="flex items-center gap-2 sm:gap-3">
            <img src={logo} alt="Firangi BBQ 2.0 Logo - Premium Tandoor & Barbeque Restaurant" className="h-10 w-10 sm:h-12 sm:w-12 rounded-full object-cover ring-2 ring-accent/40" loading="lazy" />
            <div>
              <div className="font-display text-base sm:text-xl">Firangi BBQ <span className="text-accent">2.0</span></div>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 sm:space-y-3 text-xs sm:text-sm text-muted-foreground">
            ...
            <li><a href="tel:+918588000738" className="flex items-center gap-2 sm:gap-3 hover:text-cream" aria-label="Call Firangi BBQ 2.0"><Phone className="h-3 w-3 sm:h-4 sm:w-4 text-accent flex-shrink-0" aria-hidden="true" /> +91 85880 00738</a></li>
            <li><a href="https://www.instagram.com/firangibbq2.0/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 sm:gap-3 hover:text-cream" aria-label="Visit Firangi BBQ 2.0 on Instagram"><Instagram className="h-3 w-3 sm:h-4 sm:w-4 text-accent flex-shrink-0" aria-hidden="true" /> @firangibbq2.0</a></li>
      ...
      <div className="mx-auto mt-8 sm:mt-12 max-w-7xl border-t border-border pt-4 sm:pt-6 text-xs text-muted-foreground">
        © {new Date().getFullYear()} Firangi BBQ 2.0. All rights reserved. Authentic tandoor restaurant & barbeque house in Kirti Nagar, New Delhi.
      </div>
    </footer>
  );
}
